...
function EventItem({ event, onClickFunction, showDeleteButton, showFollowButton, showUnFollowButton, currentUserID, showBookmarkButton, showUnbookmarkButton }) {
  ...
  return (
    <div className="event-item">
      ...
      {showBookmarkButton && (
        <button onClick={() => onClickFunction()}>Bookmark</button>
      )}
      {showUnbookmarkButton && (
        <button onClick={() => onClickFunction()}>Unbookmark</button>
      )}
    </div>
  );
}
